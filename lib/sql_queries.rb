...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS amt_left FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING amt_left >= 0"
end
